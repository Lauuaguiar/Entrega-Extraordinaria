...
public class SqliteWeatherStore implements WeatherStore {
    ...
    public void insertWeather(Connection connection, Weather weather, Location location, String instant) {
        if (instant == null || location == null) {
            ...
            return;}
        String tableName = location.getIsland();
        String insertQuery = "INSERT INTO " + tableName + "(Date, Temperature, Rain, Humidity, Clouds, Wind)" + " SELECT ?, ?, ?, ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM " + tableName + " WHERE Date = ?)";
    ...
    private void updateWeather(Connection connection, Weather weather, Location location) {
        if (location == null) {
            ...
            return;}
        String tableName = location.getIsland();
        String updateQuery = "UPDATE " + tableName + " SET Date = ?, Temperature = ?, Rain = ?, Humidity = ?, Clouds = ?, Wind = ? WHERE Date = ?";
    ...
    public List<Weather> getWeatherByIsland(String island, String ts, Location location) {
        List<Weather> weatherData = new ArrayList<>();
        try (Connection connection = connect()) {
            String selectQuery = "SELECT * FROM \"" + island + "\"";
            try (PreparedStatement statement = connection.prepareStatement(selectQuery)) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {String predictionTime = resultSet.getString("Date");
                        ...
                        weatherData.add(new Weather(ts, predictionTime, temperature, precipitation, wind, humidity, clouds, location));}}}
        } catch (SQLException e) {System.out.println("Error getting weather data: " + e.getMessage());}
        return weatherData;
    }
}